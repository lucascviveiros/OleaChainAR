...
public class DistancePoints : MonoBehaviour
{
	...
	private float waitTime2 = 15f; //7
	private float waitTime3 = 20f;

	/// Strings
	private string[] myLatLong;
	private string[] myActualLatLongMatrix = new string[2];
	...
	private bool oneTimeNot2 = false;
	private bool onceNotification = false;

	/// Doubles
	private double distMaisProxima = 1000000;
	private double value;
	private double[] myActualDistanceMatrix = new double[5]; //155
	private double[] myIndiceList = new double[100]; //155
	...
	private void FixedUpdate()
	{
		...
		//first timer after loading scene
		if (timer > waitTime)
		{
			timer -= waitTime; //coisa nova aq
		...
		while (timer2 >= waitTime2)
		{
			...
			/*
			int y = 0;
			while (y <= myActualDistanceMatrix.Length - 1)
            {
				Debug.Log("line: " + y + " || myActualDistanceMatrix: " + myActualDistanceMatrix[y]);
				y++;
            }

			*/

			//populateList.Populate(myActualDistanceMatrix.Length, myActualDistanceMatrix);

			///Ordenar _locationDoubles
			for(int i = 0; i<= _locationDoubles.GetLength(0) - 1; i++)
            {
				Debug.Log("i:" + i + "_LocationDoubles Lenth: " + _locationDoubles.Length);
				Debug.Log("_locationDoubles LatitudeLIST: " + _locationDoubles[i, 0]);
	...
	public void PrintKeysAndValues(SortedList myList)
	{
		...
		{
			foreach (int line in myActualDistanceMatrix)
			{

				Debug.Log("Line: " + line + " myActualDistanceMatrix: " + myActualDistanceMatrix[line]);
			}
	...
	private void KeepCalculating()
    {
		...
		for (int i = 0; i <= myActualDistanceMatrix.Length - 1; i++)
		{
			int sum = myActualDistanceMatrix.Length - 1;
			/*if (i == sum)
			{
				Debug.Log("Caiu no if__ I: " + i + " Length: " + myActualDistanceMatrix.Length);
				keepCalculating = false;
				PrintKeysAndValues(mySL);
				ResetCounters();							
			}*/
			...
			//else
			//{
			value = myActualDistanceMatrix[i];
			//Debug.Log("Distancia atual de: " + value+ " metros " + "|| indice: " + i.ToString());
			...
			if (value >= 1 && value <= 1.5)
			{
				...
				string resp = string.Format("Found:{0} . Entre 1 a 1.5 metros de distância; Value: {1}", found1_1_5, distance);
				Debug.Log("" + resp);
			...
			else if (value >= 1.5 && value <= 2)
			{
				...
				string resp = string.Format("Found:{0} . Entre 1.5 a 2 metros de distância; Value: {1}", found1_5_3, distance);
				Debug.Log("" + resp);
			}
			...
			else if (value >= 2 && value <= 3)
			{
				...
				string resp = string.Format("Found:{0} . Entre 2 a 3 metros de distância; Value: {1}", foundLess3, distance);
				Debug.Log("" + resp);
			...
			else if (value >= 5 && value <= 10)
			{
				...
				if (!oneTimeNot2)           //CallNotification()
				{
					//NotificationProviderText.text = string.Format("found!: {0} . Menos de 10 metros de distância(ACTIVATE_NOTIFICATION)", foundLess10);
					oneTimeNot2 = true;
				}
			}
			...
			else if (value >= 10 && value <= 20)
			{
				...
				if (!oneTimeNot2)           //CallNotification()
				{
					//NotificationProviderText.text = string.Format("found!: {0} . Menos de 10 metros de distância(ACTIVATE_NOTIFICATION)", found10_20);
					oneTimeNot2 = true;
				}
			}		
		}
	...
	private IEnumerator WaitDelatyCalculation()
	{
		...
		StartCoroutine(GetCurrentGPSLocation());
	...
	private IEnumerator GetCurrentGPSLocation()
	{
		//keepCalculating = true;
		...
		try
		{
			...
			doubleLat = Convert.ToDouble(myActualLatLongMatrix[0], numberFormat);
			doubleLong = Convert.ToDouble(myActualLatLongMatrix[1], numberFormat);

		}
		catch (Exception ex)
		{
			Debug.Log("algum erro");
	...
	private string[] SplitGeolocation()
	{
		currLoc = _locationProvider.CurrentLocation;
		myLatLong = currLoc.LatitudeLongitude.ToString().Split(',');
		myActualLatLongMatrix[0] = myLatLong[1].ToString();
		myActualLatLongMatrix[1] = myLatLong[0].ToString();

		return myActualLatLongMatrix;
	}
	...
	private void CalculateDistanceList(double userLat1, double userLong1)
	{
		...
		//Debug.Log("Locationn Doubles.Length: " + _locationDoubles.LongLength);

		for (int i = 0; i <= _locationDoubles.GetLength(0) - 1 ; i++)
		{ //GetLength(0) -> Gets the first dimension size

			//Debug.Log("lines: " + i);

			double mylat = _locationDoubles[i, 0];
			double mylong = _locationDoubles[i, 1];

			//Debug.Log("Lat: " + myLat);
			//Debug.Log("Long: " + mylong);

			double listLat2 = mylat;
			double listLong2 = mylong;

			double theta = userLong1 - listLong2;// - listLong;

			double myDist = Math.Sin(deg2rad(userLat1)) * Math.Sin(deg2rad(listLat2)) + Math.Cos(deg2rad(userLat1)) * Math.Cos(deg2rad(listLat2)) * Math.Cos(deg2rad(theta));
			...
			//Debug.Log("Distance: " + myDist + " metros");
			myActualDistanceMatrix[times] = myDist;
			times++;
		}
